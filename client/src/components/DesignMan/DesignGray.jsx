...
import camisetas_diseñar from "../../images/camisetas_diseñar_gris.png";
import camisetas_diseñador_detras from "../../images/camisetas_diseñador_detras_gris.png";
...
const DesignGray = () => {
  ...
  const handleAddToCart = async () => {
    ...
    try {
      ...
      // Reload the page
      alert("Diseño guardado en el carrito de compras, ingrese y realize el pedido")
    } catch (error) {
      console.log(error);
    } finally {
      window.location.reload();
...
/*    const handleSendByWhatsApp = async () => {
  ...
  return (
    <div className={styles.canvas_container}>
      ...
      <Stack
        direction="row"
        className={styles.container_all_btn}
        
     >
        <div className={styles.text_select}>
          <div>
            <label htmlFor="">Agregar texto: </label>
            <select className={styles.select} onChange={(e) => addTextWithDeleteButton(e.target.value)}>
              {fontList.map((font, index) => (
        ...
        <div>
          ...
          >
            {loadingCars ? (
              <>
             <CircularProgress color="primary"  sx={{color: "#000", fontSize: 5}}/>
             <div>+</div>
             <div>Agregar al carrito</div>
              </>
            ) : (
    ...
    <div  className={styles.drawer_mobile}>
      ...
      <Button variant="contained" sx={{background: "#000", ":hover": {background: "#000"}}} onClick={toggleDrawer(true)}>Open drawer</Button>
